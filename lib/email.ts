...
export async function sendCustomerConfirmationEmail(bookingData: BookingData) {
  try {
...
export async function sendAdminNotificationEmail(bookingData: BookingData) {
  try {
    const formattedDate = new Date(bookingData.bookingDate + "T12:00:00Z").toLocaleDateString("en-US", {
      weekday: "long",
      year: "numeric",
    ...
    const adminEmails = ["lashedbydeedeee@gmail.com", "bookings@lashedbydeedee.com"]
...
export async function sendBookingEmails(bookingData: BookingData) {
  ...
  const customerResult = await sendCustomerConfirmationEmail(bookingData)
  const adminResult = await sendAdminNotificationEmail(bookingData)
